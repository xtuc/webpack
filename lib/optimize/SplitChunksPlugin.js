...
module.exports = class SplitChunksPlugin {
	...
	static normalizeCacheGroups({ cacheGroups, automaticNameDelimiter }) {
		...
		if (cacheGroups && typeof cacheGroups === "object") {
			const fn = module => {
				let getChunksCache = undefined;
				const getChunks = () => {
					if(getChunksCache === undefined) getChunksCache = module.getChunks();
				...
				for (const key of Object.keys(cacheGroups)) {
					...
					} else if (SplitChunksPlugin.checkTest(option.test, module, getChunks)) {
						if (results === undefined) results = [];
						results.push({
							key: key,
	...
	// TODO webpack 5 remove getChunks argument
	static checkTest(test, module, getChunks) {
		if (test === undefined) return true;
		if (typeof test === "function") {
			if(test.length !== 1)
				return test(module, getChunks());
			return test(module);
		}
		if (typeof test === "boolean") return test;
		if (typeof test === "string") {
			if (module.nameForCondition && module.nameForCondition().startsWith(test)) return true;
				...
				chunks => {
					...
					const initCombinations = () => {
						...
						for (const module of compilation.modules) {
							...
							if (!chunkSetsInGraph.has(chunkIndices)) {
								chunkSetsInGraph.set(
									chunkIndices,
									new Set(module.chunksIterable)
								);
							}
						}
						...
						console.log("chunkSetsByCount", Array.from(chunkSetsByCount, p => p[0] + " = " + p[1].length));

						// prepare combinations for each set of chunk found in graph
					...
					const walkModules = () => {
						...
						for (const module of compilation.modules) {
							...
							for (const cacheGroupSource of cacheGroups) {
								...
								for (const chunkCombination of combinations.get(chunksKey)) {
									...
									const selectedChunksKey = getKey(selectedChunks);
									const key =
										(name && `name:${name}`) ||
						...
						console.timeEnd("walkModules");
					};
					walkModules();

					console.log("chunksInfoMap", chunksInfoMap.size);
