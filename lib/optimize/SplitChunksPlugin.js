				...
				chunks => {
					...
					for (const module of compilation.modules) {
						...
						for (const cacheGroupSource of cacheGroups) {
							...
							const chunksKey = getKey(module.chunksIterable);
							const comb = combinations.get(chunksKey);
							for (const chunkCombination of comb) {
								...
								if (
									chunkCombination === comb[0] &&
									info.size >= info.cacheGroup.minSize &&
									!isFinite(info.cacheGroup.maxAsyncRequests) &&
									!isFinite(info.cacheGroup.maxInitialRequests)
								) {
									// There is no restriction on this left.
									// No need to check other combinations
									// as this will be fullfilled anyway.
									break;
								}
							}
						}
